...
namespace JobSystem.BusinessLogic.Tests
{
	...
	public class JobItemTests
	{
		...
		public void UpdateStatus_InvalidJobItemId_ArgumentExceptionThrown()
		{
			var statusId = Guid.NewGuid();
			...
			_jobItemService = JobItemServiceFactory.CreateForUpdateStatus(jobItemRepositoryStub, statusId);
			UpdateJobItemStatus(statusId);
		}

		[Test]
		[ExpectedException(typeof(ArgumentException))]
		public void UpdateStatus_InvalidStatusId_ArgumentExceptionThrown()
		{
		...
		[Test]
		public void UpdateStatus_InvalidListItemType_DomainValidationExceptionThrown()
		{
			var statusId = Guid.NewGuid();
			var jobItemRepositoryStub = MockRepository.GenerateStub<IJobItemRepository>();
			jobItemRepositoryStub.Stub(x => x.GetById(_jobItemToUpdateId)).Return(_jobItemToUpdate);
			var listItemRepositoryStub = MockRepository.GenerateStub<IListItemRepository>();
			listItemRepositoryStub.Stub(x => x.GetById(statusId)).Return(new ListItem { Id = statusId, Name = "Test Status", Type = ListItemType.JobItemWorkStatus });
			_jobItemService = JobItemServiceFactory.CreateForUpdateStatus(jobItemRepositoryStub, listItemRepositoryStub);
			UpdateJobItemStatus(statusId);
			Assert.IsTrue(_domainValidationException.ResultContainsMessage(JobSystem.Resources.JobItems.Messages.InvalidListItemType));
		}

		[Test]
		public void UpdateStatus_UserHasInsufficientSecurityClearance_DomainValidationExceptionThrown()
		{
			var statusId = Guid.NewGuid();
			var jobItemRepositoryStub = MockRepository.GenerateStub<IJobItemRepository>();
			jobItemRepositoryStub.Stub(x => x.GetById(_jobItemToUpdateId)).Return(_jobItemToUpdate);
